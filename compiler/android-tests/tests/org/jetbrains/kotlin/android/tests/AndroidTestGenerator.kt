...
internal fun patchFiles(
    ...
    val saveScheme = resultFiles.any { file ->
        packages.any {
    ...
    resultFiles.forEach { file ->
        file.content = resultFiles.fold(file.content) { r, param ->
            patchClassForName(param.newClassId, param.oldPackage, r, saveScheme)
        }
    }
...
private fun patchClassForName(className: FqName, oldPackage: FqName, text: String, saveScheme: Boolean): String {
    if (!saveScheme && !oldPackage.isRoot) return text
    return text.replace(
